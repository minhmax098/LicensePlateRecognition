...
from vietho import copy_org


def aug_skew(aug_id, aug_num, org_image, image_org_filepath, image_aug_path):
    idx = 0
    # Copy image
    copy_org(image_org_filepath, image_aug_path + os.path.basename(image_org_filepath).split(".")[0] + ".png")
...
def data_augment():
    ...
    org_path = "train20x20/"
    image_org_path = org_path + "0/"

    aug_path = "train20x20_aug/"
    image_aug_path = aug_path  + "0/"
    ...
    count = 0
    # Lap qua folder data
    for img_org_path in glob.iglob(image_org_path + '*.png'):
